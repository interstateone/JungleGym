...
//
//  Based on SwiftSyntaxHighlighter.swift
...
class SwiftSyntaxHighlighter: NSObject, NSTextStorageDelegate, NSLayoutManagerDelegate {
    ...
    func parse() {
        ...
        for layoutManager in layoutManagerList {
            ...
            layoutManager.removeTemporaryAttribute(.swiftElementType, forCharacterRange: range)

            for token in tokens {
                layoutManager.addTemporaryAttributes([.swiftElementType: token.kind], forCharacterRange: token.range)
            }
        }
    }
